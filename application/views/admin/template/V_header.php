<?php
$jml_daftar = $this->db->query("SELECT count(t.transaksi_id) AS jml_daftar FROM tbl_transaksi AS t JOIN tbl_status_transaksi AS s ON t.transaksi_id = s.transaksi_order_id WHERE (s.transaksi_status IS NULL OR s.transaksi_status = '0' OR s.transaksi_status = '2')  AND t.transaksi_terima IS NULL AND t.transaksi_deleted = '0' AND s.transaksi_deleted = '0'" . $this->M_admin->tambahanQueryOrderYangFungsinyaBuatCekPermission())->row_array()['jml_daftar'];
